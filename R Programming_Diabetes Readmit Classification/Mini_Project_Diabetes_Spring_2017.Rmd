...
Factors that decrease likelihood of readmission <30 days are the following: no insulin dosage prescribed, steady dosage, or increase _all_ have a decreased effect on likelihood of short term readmission. Compared to an emergency room patient admission, both 'other' admits and referrals from home health imply a drop in likelihood of a <30 day readmission. 
...
Lastly, their are several problems, solutions, and ideas for further investigation that emerge from this dataset and these findings. 

<!---
############################################## TODO fill this innnnnn ##########################

__possible problems/complications:__
+ role of early intervention such as health eating programs and exercise regimes a la Michelle Obama's "Let's Move!" campaign. 
+ public health initiative consideration
+ environmental factors -- weather factors, walkability scores, percent car owners, average commute time, food deserts, etc. that may speak to a more sedentary lifestyle and poor eating habits.
+ with the data at hand: correlation of variables
+ better way to ascertain if it was diabetes that caused hospitalization, a concurrent/opportunistic complication, or other illness all together
+ more data from other states -- 
+ data is 1999-2008 but The ACA's major provisions came into force in 2014 -- many of these people would likely have been shut out of certain types of coverage due to pre-existing condition status during that time period. 
--->
...
For the sake of getting a better idea about the data and trends, I isolated several variables that seem likely to influence readmission. These variables are `readmitted`, `race`, `gender`, `age`, `change`, and `number_diagnoses`, I have summarized and visualized my findings below. 
...
After doing manual backwards elimination the model has 12 variables, all of which are statistically significant at the .05 level. Some of these remaining variables seem like they might exhibit colinearity. Although plotting them shows that the actuality is really not all that bad.
...
Lastly, for the sake of model robustness I am also choosing to remove `glipizide`, which is only significant at the .05 level according to the Anova test (shown below).
...
The resulting original model obtained by LASSO regularization has 9 variables, 5 of which have multiple levels: 
...
Doing a simple prediction with this model using the last observation in the dataset sets the prediction of this patient being readmitted in 30 days is 7.7%. 
...
All variables are significant at the .05 level according to the Anova test, but for robustness the two with the highest P values are being removed. After removing these two variables (`num_lab_procedures` and `max_glu_serum`) all variables are significant at the .01 level according to the Anova test. 
...
<!-- log model 2 using LASSO then forward selection -->
<!-- Call: -->
...
<!-- Deviance Residuals:  -->
<!--     Min       1Q   Median       3Q      Max   -->
<!-- -2.3954  -0.5047  -0.4300  -0.3900   2.6117   -->
